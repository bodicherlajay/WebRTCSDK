...
describe('Event emitter', function () {
  ...
  describe('Constructor', function () {
    it('should export ATT.private.createEventEmitter', function () {
      expect(ATT.private.createEventEmitter).to.be.a('function');
    });
    it('should return return an object', function () {
      var eventEmitter = ATT.private.createEventEmitter();
    ...
    describe('Different instances should not share topics', function () {
      ...
      beforeEach(function () {
        eventEmitter1 = ATT.private.createEventEmitter();
        eventEmitter2 = ATT.private.createEventEmitter();
      });
  ...
  describe.only('Methods', function () {
    ...
    beforeEach(function () {
      eventEmitter = ATT.private.createEventEmitter();
    });
    ...
    describe('subscribe', function () {
      ...
      it('should return false if `context` is `null` or if it\'s not an object', function () {
        expect(eventEmitter.subscribe('bogus', function () {}, null)).to.equal(false);
        expect(eventEmitter.subscribe('bogus', function () {}, 'anystring')).to.equal(false);
        expect(eventEmitter.subscribe('bogus', function () {}, 123)).to.equal(false);
      });

      it('should return true when context is not passed as a parameter', function () {
        expect(eventEmitter.subscribe('bogus', function () {})).to.equal(true);
        expect(eventEmitter.subscribe('bogus', function () {}, undefined)).to.equal(true);
      });

      it('should add a topic to the list of topics', function () {
        var topics,
          currentTopic = 'arun',
          eventEmitter3 = ATT.private.createEventEmitter();
    ...
    describe('unsubscribe', function () {
      ...
      it('should return false if topic not found', function () {
        expect(eventEmitter.unsubscribe('notfound', function () {})).to.equal(false);
      });
