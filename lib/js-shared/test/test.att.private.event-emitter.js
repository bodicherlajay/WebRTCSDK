  ...
  describe('Methods', function () {
    ...
    describe('unsubscribe', function () {
      ...
      it('should remove a topic when the last subscriber unsubscribes', function () {
        ...
        expect(eventEmitterToEmtpy.unsubscribe(topic, handler)).to.equal(true);
      ...
      it('should remove the subscriber from an existing topic', function () {
        var topic = "onetime",
          ...
          handler2 = function () { return;},
          eventEmitterToEmtpy = ATT.private.factories.createEventEmitter();
        eventEmitterToEmtpy.subscribe(topic, handler1);
        eventEmitterToEmtpy.subscribe(topic, handler2);
        expect(eventEmitterToEmtpy.unsubscribe(topic, handler1)).to.equal(true);
        expect(eventEmitterToEmtpy.getTopics()[topic].length).to.equal(1);
      });
