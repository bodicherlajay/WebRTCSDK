  ...
  describe('Log Manager Singleton', function () {
    ...
    describe('Logger', function () {
      ...
      it('should log a DEBUG message with filename and line number', function () {
        ...
        positionInfo = spy.getCall(0).args[0];
        fileName = positionInfo.split(':')[0];
        try {
          lineNo = parseInt(positionInfo.split(':')[1], 10);
        } catch (e) {
          throw e;
        }

        expect(fileName).to.match(/LogManagerModuleTest[.]js/);
        expect(lineNo).to.be.a('number');
